...
単純なリストの場合、以下を想定してますが、オーム社さんの制約があるか調整中です。

- ひとつめ
- ふたつめ
- みっつめ

先頭太文字の場合、以下のルールで考えてます。Markdown記法から外れるのでオーム社さんと調整中です。
...
以下のように原著で段落レベルで明らかに引用という個所にはmarkdown形式の引用表現を使用する。参考文献は[Name]として、原著と同じ表現とする。人名は原著のまま英語で表現。文書内の書籍の引用は「* 
」で囲みます。
...
- 節、項タイトルは↑のタグのみ。文中の索引はさらに太字にする：「**」で囲む。
...
## X.4 原著注釈、訳注、人名、参考文献、図（調整中）

### X.4.1 原著注釈、訳注、人名、参考文献（まとめて記述したサンプル）

以下のように原著注釈、人名、参考文献の表現を行う。（調整中です。あとで修正するかも）

数値のインクリメントは章単位で行ってください。

ソフトウェアプロセスが存在するからといって、ソフトウェアが納期に納品され、顧客ニーズを満たし、長期的な品質を確保できるという保証はない（第15章参照）。<span class="index">**プロセスパターン**</span>[1]は確固たるソフトウェアエンジニアリング（第2部参照）の実践と共に存在しなければならない。さらに、ソフトウェアエンジニアリングを成功させるために必要不可欠な基本的なプロセス基準を確実に満たすよう、プロセス自体も評価される必要がある[2]。

[1] [訳注1] プロセスパターンは2.5節や第3章（アジャイル開発プロセス）等で紹介される各プロセスモデルの総称。
...
以下のように上下を「^^^^」で囲む。原著の補足説明ボックス（タスクセットなど）内容を間に記載する。
...
表に関してはExcelなりで保存ください。[こちらのディレクトリ](./doc_sample)の「chXX.xlsx」のように作成し、mdを同じディレクトリに置きます。

文中には以下のようにコードブロックで記述します。

```
表X.1 プロセスモデルのポジティブ要素とネガティブ要素
